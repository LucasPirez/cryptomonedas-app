...
export const AppContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (query.id) {
      ...
      if (
        asPath.includes("exchanges") &&
        numberActualState.exchanges !== Number(query.id)
      ) {
        (async () => {
          const responseList = await exchangesList(query.id);
          const resultList = await responseList.json();

          setNumberActualState({
            ...numberActualState,
            exchanges: Number(query.id),
          });
          setDataExchanges({ ...dataExchanges, datos: resultList });
        })();
      }
    }
